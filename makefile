...
CC = g++
EXEC = emerald
CCFLAGS = -std=c++11 -DGL_GLEXT_PROTOTYPES -DGLEW_STATIC -DGLM_FORCE_RADIANS -D_DEFAULT_SCENE_FILE_="\"res/scenes/scene_file.txt\"" -D$(FRUSTUM_NEAR) -D$(FRUSTUM_FAR) -D$(SCREEN_HEIGHT) -D$(SCREEN_WIDTH) -D$(SSAO_N_SAMPLES) #-D_DEBUG_LOADER_
INC =
LDFLAGS = -lSDL2 -lSDL2_image -lassimp
FRAGMENT_SHADER_PRECOMPILE= $(MAX_LIGHTS) $(FRUSTUM_NEAR) $(FRUSTUM_FAR) $(SCREEN_WIDTH) $(SCREEN_HEIGHT) $(SSAO_N_SAMPLES)
