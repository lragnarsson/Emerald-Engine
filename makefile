...
CC = g++
EXEC = emerald
CCFLAGS = -std=c++11 -DGL_GLEXT_PROTOTYPES -DGLEW_STATIC -DGLM_FORCE_RADIANS -D_DEFAULT_SCENE_FILE_="\"res/scenes/terrain_demo.txt\"" -D$(FRUSTUM_NEAR) -D$(FRUSTUM_FAR) -D$(EMERALD_SCREEN_HEIGHT) -D$(EMERALD_SCREEN_WIDTH) -D$(SSAO_N_SAMPLES) -D$(ATT_QUAD) -D$(ATT_LIN) -D$(ATT_CON) -D$(MAX_LIGHTS) -D$(MAX_MODELS) -D$(SSAO_SCALE) -D$(SHADOW_SCALE) -D$(MAX_TERRAIN_MESHES) #-DPROFILING -D_DEBUG_LOADER_
INC =
