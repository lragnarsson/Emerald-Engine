...
CC = g++
EXEC = emerald
CCFLAGS = -std=c++11 -DGL_GLEXT_PROTOTYPES -DGLEW_STATIC -DGLM_FORCE_RADIANS -D_DEFAULT_SCENE_FILE_="\"res/scenes/scene_file.txt\"" -D$(FRUSTUM_NEAR) -D$(FRUSTUM_FAR) -D$(SCREEN_HEIGHT) -D$(SCREEN_WIDTH) -D$(SSAO_N_SAMPLES) -D$(ATT_QUAD) -D$(ATT_LIN) -D$(ATT_CON) -D$(MAX_LIGHTS) -D$(SSAO_SCALE) -DPROFILING -D$(SHADOW_SCALE) #-D_DEBUG_LOADER
